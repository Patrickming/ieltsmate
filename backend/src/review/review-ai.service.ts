import { Injectable, Logger } from '@nestjs/common'
import { AiService } from '../ai/ai.service'
import { FreeDictionaryApiService } from '../dictionary/free-dictionary-api.service'
import { NotesService } from '../notes/notes.service'
import { enrichReviewCardWithDictionary } from './review-dictionary-enrichment'
import type { ConfusableGroup } from '../notes/types/note-extensions'
import { PrismaService } from '../prisma/prisma.service'
import { parseReviewAiPayload } from './review-ai-content.util'
import {
  CardAIContent,
  CardType,
  FallbackResponse,
} from './types/card-ai-content'

/** 单张复习卡 AI 生成在服务端等待 LLM 的上限（毫秒）。 */
const REVIEW_AI_GENERATION_TIMEOUT_MS = 30_000

@Injectable()
export class ReviewAiService {
  private readonly logger = new Logger(ReviewAiService.name)
  /** 基于编辑距离与较长词长的比值；略放宽以保留 consult/insult、result/consult 等常见形近对。 */
  private static readonly MIN_FORM_CONFUSABLE_SIMILARITY = 0.55

  constructor(
    private readonly prisma: PrismaService,
    private readonly aiService: AiService,
    private readonly dictionary: FreeDictionaryApiService,
    private readonly notesService: NotesService,
  ) {}

  private extractFencedJson(content: string): string | null {
    const fenceMatch = content.match(/```json\s*([\s\S]*?)```/i) ?? content.match(/```\s*([\s\S]*?)```/)
    if (!fenceMatch) {
      return null
    }
    const jsonText = fenceMatch[1].trim()
    return jsonText ? jsonText : null
  }

  /**
   * 扫描文本中首个“平衡”的 JSON 对象，能正确跳过字符串内花括号与转义字符。
   */
  private extractFirstBalancedJsonObject(content: string): string | null {
    let start = -1
    let depth = 0
    let inString = false
    let escaped = false

    for (let i = 0; i < content.length; i += 1) {
      const ch = content[i]

      if (start === -1) {
        if (ch === '{') {
          start = i
          depth = 1
          inString = false
          escaped = false
        }
        continue
      }

      if (escaped) {
        escaped = false
        continue
      }

      if (ch === '\\') {
        escaped = true
        continue
      }

      if (ch === '"') {
        inString = !inString
        continue
      }

      if (inString) {
        continue
      }

      if (ch === '{') {
        depth += 1
        continue
      }

      if (ch === '}') {
        depth -= 1
        if (depth === 0) {
          return content.slice(start, i + 1)
        }
      }
    }

    return null
  }

  private extractJsonCandidate(content: string): string {
    const fenced = this.extractFencedJson(content)
    if (fenced) {
      return fenced
    }
    const balanced = this.extractFirstBalancedJsonObject(content)
    if (balanced) {
      return balanced
    }
    return content
  }

  private normalizeWordSurface(word: string): string {
    return word.trim().toLowerCase()
  }

  private editDistance(a: string, b: string): number {
    const rows = a.length + 1
    const cols = b.length + 1
    const dp = Array.from({ length: rows }, () => Array<number>(cols).fill(0))

    for (let i = 0; i < rows; i += 1) {
      dp[i][0] = i
    }
    for (let j = 0; j < cols; j += 1) {
      dp[0][j] = j
    }

    for (let i = 1; i < rows; i += 1) {
      for (let j = 1; j < cols; j += 1) {
        const cost = a[i - 1] === b[j - 1] ? 0 : 1
        dp[i][j] = Math.min(
          dp[i - 1][j] + 1,
          dp[i][j - 1] + 1,
          dp[i - 1][j - 1] + cost,
        )
      }
    }

    return dp[a.length][b.length]
  }

  private formConfusableSimilarity(target: string, candidate: string): number {
    const maxLen = Math.max(target.length, candidate.length)
    if (maxLen === 0) {
      return 1
    }
    return 1 - this.editDistance(target, candidate) / maxLen
  }

  private getAssociationWordSet(content: CardAIContent): Set<string> {
    const set = new Set<string>()
    if (!('synonyms' in content) || !Array.isArray(content.synonyms)) {
      return set
    }
    for (const item of content.synonyms) {
      if (item && typeof item === 'object' && 'word' in item && typeof item.word === 'string') {
        set.add(this.normalizeWordSurface(item.word))
      }
    }
    if ('antonyms' in content && Array.isArray(content.antonyms)) {
      for (const item of content.antonyms) {
        if (item && typeof item === 'object' && 'word' in item && typeof item.word === 'string') {
          set.add(this.normalizeWordSurface(item.word))
        }
      }
    }
    return set
  }

  private isTargetAnchoredConfusable(
    group: ConfusableGroup,
    targetSurface: string,
    associationWordSet: Set<string>,
  ): boolean {
    const surfaces = group.words.map((item) => this.normalizeWordSurface(item.word))
    if (!surfaces.includes(targetSurface)) {
      return false
    }

    const candidateSurfaces = surfaces.filter((surface) => surface !== targetSurface)
    if (group.kind === 'meaning') {
      return candidateSurfaces.every((surface) => !associationWordSet.has(surface))
    }

    return candidateSurfaces.every((surface) =>
      this.formConfusableSimilarity(targetSurface, surface) >=
      ReviewAiService.MIN_FORM_CONFUSABLE_SIMILARITY,
    )
  }

  private sanitizeConfusablesForTarget<T extends CardAIContent>(
    content: T,
    targetWord: string,
  ): T {
    if (!('confusables' in content) || !Array.isArray(content.confusables)) {
      return content
    }

    const targetSurface = this.normalizeWordSurface(targetWord)
    if (!targetSurface) {
      return content
    }
    const associationWordSet = this.getAssociationWordSet(content)
    associationWordSet.delete(targetSurface)

    const filtered = content.confusables.filter((group) =>
      this.isTargetAnchoredConfusable(group, targetSurface, associationWordSet),
    )

    if (filtered.length === content.confusables.length) {
      return content
    }

    const next = { ...content } as T & { confusables?: ConfusableGroup[] }
    if (filtered.length > 0) {
      next.confusables = filtered
    } else {
      delete next.confusables
    }
    return next as T
  }

  async generate(noteId: string, cardType: CardType): Promise<CardAIContent> {
    const note = await this.prisma.note.findUnique({ where: { id: noteId } })
    if (!note) {
      return this.buildFallback(null, '笔记不存在')
    }

    const prompt = this.buildPrompt(note, cardType)

    try {
      // 使用 complete（无 tools）：chat() 会附带 tools 与多轮 function calling，
      // 易出现非纯 JSON / 占位文案导致 parseAIResponse 解析失败。
      const raw = await this.aiService.complete({
        messages: [{ role: 'user', content: prompt }],
        slot: 'review',
        timeoutMs: REVIEW_AI_GENERATION_TIMEOUT_MS,
      })

      const parsed = this.parseAIResponse(raw, cardType, note)
      const enriched = await enrichReviewCardWithDictionary(
        this.dictionary,
        parsed,
        cardType,
        note.content,
      )
      void this.notesService.saveBritishPronunciationToNote(noteId, note.content)
      return enriched
    } catch (err) {
      this.logger.warn(`AI generation failed for note ${noteId}: ${String(err)}`)
      const fallback = this.buildFallback(note, 'AI 调用失败或超时')
      const enriched = await enrichReviewCardWithDictionary(
        this.dictionary,
        fallback,
        cardType,
        note.content,
      )
      void this.notesService.saveBritishPronunciationToNote(noteId, note.content)
      return enriched
    }
  }

  private buildPrompt(
    note: {
      content: string
      translation: string
      category: string
      phonetic?: string | null
      synonyms: string[]
      antonyms: string[]
      example?: string | null
      memoryTip?: string | null
    },
    cardType: CardType,
  ): string {
    const base = `单词/内容: "${note.content}"`
    /** 不向模型提供用户录入的中文释义，避免错误释义牵引生成结果。 */
    const englishOnlyGuard =
      '重要：本提示不包含用户笔记中的中文释义。请仅依据上述英文词、短语或句子本身，按权威词典与真实语料判断词性、义项并生成全部内容；输出字段中的中文说明须准确、规范。'
    const confusableGuard =
      'confusables 必须围绕目标词：每个分组 words 里都须含目标词英文原文，不要两个都不是目标词的对比。' +
      '可返回多个分组：form（形近/拼写易混）与 meaning（义近/用法易混）各自都可以多于 1 组；建议尽量给出至少 1 组 form、至少 1 组 meaning（若确实存在易混点）。' +
      'form 类：指「拼写或视觉上容易看错、写混」的配对，例如末尾相同/少数字母不同（cite/site、ensure/insure、whether/weather、consult/insult、strip/stripe）；不要仅因发音或主题相关就放进 form；同词根派生属于 wordFamily，不要标成 form。' +
      '每组 form 仅含目标词与一个易混词；若存在多组不同的形近易混词，应分别用多个 kind 为 form 的分组列出。' +
      'meaning 类：须非空 difference；不要把已出现在 synonyms/antonyms 的词再塞进 meaning 组；避免纯上下位或仅反义关系。'
    const associationLimitGuard = '关联词数组需控制在 1-6 项：每个数组最多返回 6 项（建议 3-5 项），避免冗长堆砌。'
    const noPhoneticGuard =
      '不要返回 phonetic 字段；不要为 wordFamily 项填写音标（音标与英式读音由词典 API 单独提供）。'

    if (cardType === 'word-speech') {
      return `${base}\n\n${englishOnlyGuard}\n${noPhoneticGuard}\n你是一位专业的IELTS词汇教师。请为这个单词生成复习卡片背面内容，以JSON格式返回：
{
  "fallback": false,
  "synonyms": [
    { "word": "同义词1", "meaning": "该同义词相对目标词的具体中文释义" },
    { "word": "同义词2", "meaning": "具体中文释义" },
    { "word": "同义词3", "meaning": "具体中文释义" }
  ],
  "antonyms": [
    { "word": "反义词1", "meaning": "具体中文释义" },
    { "word": "反义词2", "meaning": "具体中文释义" }
  ],
  "example": "一个地道的例句",
  "exampleTranslation": "例句的中文翻译",
  "memoryTip": "帮助记忆的技巧或联想",
  "partsOfSpeech": [
    { "pos": "词性缩写如 n./v.", "label": "词性中文如 名词", "meaning": "该词性下的义项（中文）" }
  ],
  "confusables": [
    {
      "kind": "form",
      "words": [
        { "word": "目标词", "meaning": "目标词义项（中文）" },
        { "word": "第一组形近易混词", "meaning": "义项（中文）" }
      ]
    },
    {
      "kind": "form",
      "words": [
        { "word": "目标词", "meaning": "目标词义项（中文）" },
        { "word": "第二组形近易混词", "meaning": "义项（中文）" }
      ]
    },
    {
      "kind": "meaning",
      "difference": "易混点核心区别说明（中文，必填）",
      "words": [
        { "word": "目标词", "meaning": "目标词义项" },
        { "word": "义近易混词", "meaning": "义项" }
      ]
    }
  ],
  "wordFamily": {
    "base": { "word": "目标词", "pos": "adjective", "meaning": "义项（中文）" },
    "derivedByPos": {
      "noun": [{ "word": "派生名词", "pos": "noun", "meaning": "义项" }],
      "verb": [],
      "adjective": [],
      "adverb": []
    },
    "rootDerived": [
      { "word": "同词根关联词", "pos": "verb", "meaning": "义项" }
    ]
  }
}
partsOfSpeech 必填，穷举该词所有真实存在的词性及义项（依据词典判断，若确实只有一种词性则只返回该词性）；confusables/wordFamily 可选；wordFamily.base 必填（若提供 wordFamily）；derivedByPos 四个数组均需存在；derivedByPos 只放词形变化派生（加前缀/后缀改变词性，意思与目标词紧密相关）；rootDerived 放同词根/词缀但意思已分化的关联词（平铺数组，不按词性分组）；两者不要重复。wordFamily.base.pos 必须是“目标词本身”的主词性（不要把派生词词性当作 base）；派生项 pos 必须为 noun|verb|adjective|adverb 且与所在分区一致；confusables 中 kind 为 form 或 meaning；kind 为 meaning 时必须提供非空 difference；每组至少两个 words；form 可有多组以覆盖多个形近词。
${confusableGuard}
synonyms 与 antonyms 须为对象数组 [{ "word": "…", "meaning": "…" }]：word 与 meaning 必须非空；meaning 必须给出具体中文释义。${associationLimitGuard}只返回 JSON，不要其他内容。`
    }

    if (cardType === 'phrase') {
      return `${base}\n\n${englishOnlyGuard}\n${noPhoneticGuard}\n你是一位专业的IELTS词汇教师。请为这个短语生成复习卡片背面内容，以JSON格式返回：
{
  "fallback": false,
  "synonyms": [
    { "word": "同义短语1", "meaning": "该短语相对目标短语的具体中文释义" },
    { "word": "同义短语2", "meaning": "具体中文释义" }
  ],
  "antonyms": [
    { "word": "反义短语1", "meaning": "具体中文释义" },
    { "word": "反义短语2", "meaning": "具体中文释义" }
  ],
  "example": "一个地道的例句",
  "exampleTranslation": "例句的中文翻译",
  "memoryTip": "帮助记忆的技巧"
}
synonyms 与 antonyms 须为对象数组 [{ "word": "…", "meaning": "…" }]：word 与 meaning 必须非空；meaning 必须给出具体中文释义。${associationLimitGuard}只返回 JSON，不要其他内容。`
    }

    if (cardType === 'synonym') {
      const words = note.content
        .split(/[,，/、]/)
        .map((w) => w.trim())
        .filter(Boolean)
      const wordList = words.map((w) => `"${w}"`).join(', ')
      return `同义词组: [${wordList}]\n\n${englishOnlyGuard}\n你是一位专业的IELTS词汇教师。请为这个同义替换词组生成复习卡片背面内容，以JSON格式返回：
{
  "fallback": false,
  "wordMeanings": [
    { "word": "单词1", "meaning": "具体含义及与其他词的细微差别" },
    { "word": "单词2", "meaning": "具体含义及与其他词的细微差别" }
  ],
  "antonymGroup": [
    { "word": "反义替换词1", "meaning": "具体中文释义" },
    { "word": "反义替换词2", "meaning": "具体中文释义" },
    { "word": "反义替换词3", "meaning": "具体中文释义" }
  ],
  "moreSynonyms": [
    { "word": "更多同义词1", "meaning": "具体中文释义" },
    { "word": "更多同义词2", "meaning": "具体中文释义" },
    { "word": "更多同义词3", "meaning": "具体中文释义" }
  ]
}
${noPhoneticGuard} antonymGroup 与 moreSynonyms 须为对象数组 [{ "word": "…", "meaning": "…" }]：word 与 meaning 必须非空；meaning 必须给出具体中文释义。${associationLimitGuard}只返回 JSON，不要其他内容。`
    }

    if (cardType === 'sentence') {
      return `原句: "${note.content}"\n\n${englishOnlyGuard}\n你是一位专业的IELTS写作教师。请生成复习卡片背面内容，以JSON格式返回：
{
  "fallback": false,
  "analysis": "帮助理解句意的自然语言解析（重点说明句子逻辑结构、关键短语的作用，使学习者能读懂这句话）",
  "paraphrases": [
    { "sentence": "改写版本1（替换不同结构）", "dimension": "结构变化：如从句→分词" },
    { "sentence": "改写版本2（替换不同词汇）", "dimension": "词汇变化：如词性转换" },
    { "sentence": "改写版本3（整体重构）", "dimension": "整体重构：完全不同的表达方式" }
  ]
}
只返回 JSON，不要其他内容。`
    }

    if (cardType === 'spelling') {
      return `单词: "${note.content}"\n\n${englishOnlyGuard}\n${noPhoneticGuard}\n你是一位专业的IELTS词汇教师。请为拼写练习生成复习卡片背面内容，以JSON格式返回：
{
  "fallback": false,
  "synonyms": [
    { "word": "同义词1", "meaning": "具体中文释义" },
    { "word": "同义词2", "meaning": "具体中文释义" }
  ],
  "antonyms": [
    { "word": "反义词1", "meaning": "具体中文释义" },
    { "word": "反义词2", "meaning": "具体中文释义" }
  ],
  "memoryTip": "拼写记忆技巧（如词根词缀分析）",
  "contextExample": {
    "sentence": "包含该单词的例句",
    "translation": "例句的中文翻译",
    "analysis": "例句结构分析和理解要点"
  },
  "partsOfSpeech": [
    { "pos": "词性缩写", "label": "词性中文", "meaning": "义项（中文）" }
  ],
  "confusables": [
    {
      "kind": "form",
      "words": [
        { "word": "目标词", "meaning": "目标词义项" },
        { "word": "第一组形近易混词", "meaning": "义项" }
      ]
    },
    {
      "kind": "form",
      "words": [
        { "word": "目标词", "meaning": "目标词义项" },
        { "word": "第二组形近易混词", "meaning": "义项" }
      ]
    },
    {
      "kind": "meaning",
      "difference": "核心区别说明（必填）",
      "words": [
        { "word": "目标词", "meaning": "目标词义项" },
        { "word": "义近易混词", "meaning": "义项" }
      ]
    }
  ],
  "wordFamily": {
    "base": { "word": "目标词", "pos": "verb", "meaning": "义项（中文）" },
    "derivedByPos": {
      "noun": [],
      "verb": [],
      "adjective": [{ "word": "derived", "pos": "adjective", "meaning": "义项" }],
      "adverb": []
    },
    "rootDerived": [
      { "word": "同词根关联词", "pos": "verb", "meaning": "义项" }
    ]
  }
}
partsOfSpeech 必填，穷举该词所有真实存在的词性及义项（依据词典判断，若确实只有一种词性则只返回该词性）；confusables/wordFamily 可选；wordFamily 规则同 word-speech 卡片说明；confusables 规则同 word-speech 卡片说明；form 可有多组。
${confusableGuard}
synonyms 与 antonyms 须为对象数组 [{ "word": "…", "meaning": "…" }]：word 与 meaning 必须非空；meaning 必须给出具体中文释义。${associationLimitGuard}只返回 JSON，不要其他内容。`
    }

    return `${base}\n请以JSON格式提供学习内容。`
  }

  private parseAIResponse(
    content: string,
    cardType: CardType,
    note: {
      content: string
      translation: string
      phonetic?: string | null
      synonyms: string[]
      antonyms: string[]
      example?: string | null
      memoryTip?: string | null
    },
  ): CardAIContent {
    try {
      const jsonStr = this.extractJsonCandidate(content)
      const parsed = JSON.parse(jsonStr.trim()) as unknown

      if (
        parsed &&
        typeof parsed === 'object' &&
        (parsed as { fallback?: boolean }).fallback === false
      ) {
        const contentParsed = parseReviewAiPayload(parsed, cardType)
        if (contentParsed !== null) {
          return this.sanitizeConfusablesForTarget(contentParsed, note.content)
        }
      }

      return this.buildFallback(note, 'AI 返回格式不符合要求')
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err)
      const preview = content.slice(0, 500).replace(/\s+/g, ' ')
      this.logger.warn(
        `Review AI JSON parse/extract failed: ${msg} · cardType=${cardType} · preview=${preview || '(empty)'}`,
      )
      return this.buildFallback(note, '无法解析 AI 输出')
    }
  }

  private buildFallback(
    note: {
      translation: string
      phonetic?: string | null
      synonyms: string[]
      antonyms: string[]
      example?: string | null
      memoryTip?: string | null
    } | null,
    reason: string,
  ): FallbackResponse {
    return {
      fallback: true,
      reason,
      phonetic: note?.phonetic ?? null,
      translation: note?.translation ?? '',
      synonyms: note?.synonyms ?? [],
      antonyms: note?.antonyms ?? [],
      example: note?.example ?? null,
      memoryTip: note?.memoryTip ?? null,
    }
  }
}
