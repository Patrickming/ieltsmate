...
const SAVE_NOTICE_STYLE: Record<
  SaveNoticeTone,
        ...
        <div className="flex-1 flex flex-col justify-center items-center gap-5 px-8 pb-5 min-h-0">
          ...
          <div className="flex flex-col items-center gap-2 w-full max-w-[560px]">
            ...
            <p className="text-[11px] text-text-subtle">
              输入后按{" "}
              <kbd className="px-1.5 py-0.5 rounded bg-[#27272a] border border-[#3f3f46] text-[10px] text-text-dim font-mono">
                Enter
              </kbd>{" "}
              /
              <kbd className="ml-1 px-1.5 py-0.5 rounded bg-[#27272a] border border-[#3f3f46] text-[10px] text-text-dim font-mono">
                Space
              </kbd>{" "}
              或点击卡片翻转查看
            </p>
...
interface SynonymAI extends AIContentBase {
  fallback: false;
  wordMeanings: Array<{ word: string; phonetic: string; meaning: string }>;
  antonymGroup: AssociationPair[];
  moreSynonyms: AssociationPair[];
}
interface SentenceAI extends AIContentBase {
  fallback: false;
  analysis: string;
...
interface SpellingAI extends AIContentBase {
  ...
  antonyms: AssociationPair[];
  memoryTip: string;
  contextExample: { sentence: string; translation?: string; analysis: string };
  partsOfSpeech?: PartOfSpeechItem[];
  confusables?: ConfusableGroup[];
  wordFamily?: WordFamily;
}
...
function buildDefaultWordFamily(note: Note): WordFamily {
  const firstMeaning =
    note.translation
      .split(/[；;]/)
      .map((s) => s.trim())
      .filter(Boolean)[0] ?? note.translation;
  return {
    base: {
      word: note.content,
      pos: "other",
...
function ReviewBackTabBar({
  ...
  return (
    <div
      className="flex gap-2 mb-4 shrink-0 flex-wrap"
      onClick={(e) => e.stopPropagation()}
    >
      {btn("learning", "review-back-tab-learning", "学习内容")}
      {showWordFamily
        ? btn("wordFamily", "review-back-tab-word-family", "词义&词根派生")
        : null}
...
function getMergedPartOfSpeechItems({
  note,
  aiPos,
  aiWordFamily,
  storedNote,
  ...
  const sourceWf = aiWordFamily ?? storedNote.wordFamily;
    ...
    return [
      {
        ...
        meaning,
        ...(sourceWf.base.phonetic?.trim()
          ? { phonetic: sourceWf.base.phonetic.trim() }
          : {}),
      },
    ];
  })();

  return mergeUniquePos(
    allStoredPos,
    mergeUniquePos(allAiPos, basePosFromWordFamily),
  );
}

function PartOfSpeechMeaningBlock({
  note,
  aiPos,
  aiWordFamily,
  storedNote,
  ...
  const barColor = CATEGORY_BAR[note.category] ?? "#818cf8";
  const mergedPos = getMergedPartOfSpeechItems({
    note,
    aiPos,
    aiWordFamily,
    storedNote,
  ...
  if (mergedPos.length === 0) return null;

  return (
    <div>
      <div className="text-sm font-semibold text-text-muted mb-2">完整词义</div>
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-2 mb-2">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="text-[12px] text-text-subtle font-mono">
                    {item.pos}
                  </span>
                </div>
                {!readOnly && (
...
function PosConfusablePanel({
  ...
  const confSurfaceCount = mergedConf.reduce<Record<string, number>>(
    (acc, g) => {
        ...
        <div>
          ...
          <div className="flex flex-col gap-4">
            {mergedConf.map((group, gi) => {
              ...
              const isCrossKindDuplicate =
                (confSurfaceCount[confusableWordsSurfaceKey(group)] ?? 0) > 1;
              return (
                <div
...
function isWordFamilyItemSaved(
  ...
  if (!wf) return false;
  return flattenWordFamilyItems(wf).some(
    (x) => wordFamilyItemDedupKey(x) === k,
  );
}
...
function WordFamilyPanel({
  ...
  return (
    <div className="flex flex-col gap-5" onClick={(e) => e.stopPropagation()}>
      <div className="text-base font-bold text-text-secondary mb-2">
        词义派生
      </div>
      ...
      {posKeys.map((pk) => (
        <div key={pk}>
          ...
          {merged.derivedByPos[pk].filter(
            (item) => item.word.trim().toLowerCase() !== currentWordSurface,
          ).length === 0 ? (
            <p
              className="text-sm text-text-dim"
              data-testid={`review-wf-empty-${pk}`}
            >
              无
            </p>
          ) : (
            <div className="flex flex-col gap-3">
              {merged.derivedByPos[pk]
                .filter(
                  (item) =>
                    item.word.trim().toLowerCase() !== currentWordSurface,
                )
                .map((item, idx) => {
                  const saved = isWordFamilyItemSaved(
                    item,
                    storedNote,
                    savedKeys,
                  );
      ...
      {mergedRoot.filter(
        ...
        <p
          className="text-sm text-text-dim"
          data-testid="review-root-derived-empty"
        >
          无
        </p>
      ) : (
...
function CardBackWordPhrase({
  ...
  return (
    <div
      className="flex flex-col gap-5 p-7 overflow-y-auto h-full"
      onClick={(e) => e.stopPropagation()}
    >
      ...
      {showReviewTabs && (
        <ReviewBackTabBar
          active={backFaceTab}
          onChange={onBackFaceTabChange!}
          showWordFamily={showWordFamilyTab}
        />
      )}
      {showReviewTabs &&
      showWordFamilyTab &&
      backFaceTab === "wordFamily" &&
      storedNote &&
      onSavePos &&
      onSaveWordFamilyItem &&
      onSaveWordFamilyAll ? (
        ...
        <>
          ...
          <div>
            ...
            <p className="text-[17px] text-text-primary leading-snug">
              {note.translation}
            </p>
          </div>
          {showReviewTabs && showWordFamilyTab && storedNote && onSavePos && (
            <PartOfSpeechMeaningBlock
              note={note}
              aiPos={extPos}
              aiWordFamily={speechAi.wordFamily}
              storedNote={storedNote}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 mt-2.5">
                {ai.synonyms.map((s, i) => {
                  ...
                  const chipSaved =
                    savedSyn.includes(formatted) ||
                    associationDisplaySaved(formatted, existingSynList);
                  return (
                    <SaveChip
                      key={`${formatted}-${i}`}
                      pair={s}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 mt-2.5">
                {ai.antonyms.map((s, i) => {
                  ...
                  if (!formatted) return null;
                  const chipSaved =
                    savedAnt.includes(formatted) ||
                    associationDisplaySaved(formatted, existingAntList);
                  return (
                    <SaveChip
                      key={`${formatted}-${i}`}
                      pair={s}
          ...
          {ai.memoryTip && (
            ...
            >
              <AITip
                label="✨ 记忆技巧"
                tip="AI 基于词源/联想生成，帮助加深印象"
              />
              <p className="text-[13px] text-[#c4b5fd] leading-relaxed mt-2">
                {ai.memoryTip}
              </p>
            </div>
          )}
  ...
  return (
    ...
    >
      ...
      <div>
        <AITip
          label="各词细微差别"
          tip="点击「存入」可将该词（含中文释义）保存到知识库同义词"
        />
              ...
              >
                <div className="flex items-start justify-between gap-2 mb-1.5">
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      onSaveSyn({ word: wm.word, meaning: wm.meaning })
                    }
      ...
      {ai.antonymGroup.length > 0 && (
        <div>
          <AITip
            label="🔀 反义同义替换"
            tip="AI 生成的语义对立替换词组，点击「存入」可保存到知识库反义词"
          />
          <div className="flex flex-wrap gap-2 mt-2.5">
      ...
      {ai.moreSynonyms.length > 0 && (
        <div>
          <AITip
            label="➕ 更多同义替换"
            tip="AI 扩展生成的近义词组，点击「存入」可保存到知识库同义词"
          />
          <div className="flex flex-wrap gap-2 mt-2.5">
          ...
          <div className="flex flex-col gap-3 mt-2.5">
            {ai.paraphrases.map((p, i) => (
              ...
              >
                ...
                <p className="text-[14px] text-text-secondary leading-relaxed mt-1.5 italic">
                  "{p.sentence}"
                </p>
              </div>
            ))}
        ...
        <>
              ...
              {!spellingCorrect && (
                <>
                  ...
                  <div>
                    ...
                    <p className="text-[17px] font-mono font-semibold text-[#34d399]">
                      {note.content}
                    </p>
                  </div>
                </>
          ...
          <div>
            ...
            <div className="bg-[#141420] border border-[#27272a] rounded-xl px-4 py-3.5 mt-2.5 flex flex-col gap-2">
              ...
              <p className="text-[12px] text-text-subtle leading-relaxed border-t border-[#27272a] pt-2 mt-1">
                {ai.contextExample.analysis}
              </p>
            </div>
          </div>
        </>
...
function CardBackFallback({
  ...
  const spellingCorrect =
    cardType === "spelling" && spellingAnswer
      ? spellingAnswer.trim().toLowerCase() ===
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2 px-3 py-2 rounded-lg bg-yellow-950/30 border border-yellow-800/40">
        <div className="flex items-center gap-2 text-[12px] text-yellow-400">
          <span>⚠</span>
          <span>
            AI 内容生成失败（已自动重试 {AI_GENERATE_AUTO_RETRIES}{" "}
            次），显示基础内容
          </span>
        </div>
        {onRetry && (
      ...
      onSaveWordFamilyAll ? (
        ...
        <>
          ...
          {showExtTabs && showWordFamilyTab && storedNote && onSavePos && (
            <PartOfSpeechMeaningBlock
              note={note}
              aiPos={undefined}
              aiWordFamily={undefined}
          ...
          {((note.synonyms?.length ?? 0) > 0 ||
            (note.antonyms?.length ?? 0) > 0) && (
            <div className="h-px bg-border" />
          )}
          {note.synonyms && note.synonyms.length > 0 && (
            <div>
              <div className="text-sm font-semibold text-text-dim mb-2">
                🔄 同义词/短语
              </div>
          ...
          {note.memoryTip && (
            <div
              ...
              style={{ background: "#1e1a2e", borderColor: barColor + "40" }}
            >
              <div className="text-sm font-semibold text-text-dim mb-1">
                ✨ 记忆技巧
              </div>
    ...
    >
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2 text-right shrink-0">
          <span className="text-[10px] text-text-subtle px-2 py-0.5 rounded-md border border-border bg-[#1a1a22]">
            快速复习
          </span>
          <span className="text-sm text-text-dim">已翻转</span>
        </div>
      ...
      {cardType === "synonym" && (
        <div>
          ...
          <p className="text-[17px] text-text-primary leading-snug font-mono">
            {note.content}
          </p>
        </div>
      )}
      ...
      {cardType === "sentence" && (
        <div>
          ...
          <p className="text-[15px] text-text-secondary italic leading-relaxed">
            "{note.content}"
          </p>
        </div>
      )}
      ...
      {((n.synonyms?.length ?? 0) > 0 || (n.antonyms?.length ?? 0) > 0) && (
        <div className="h-px bg-border" />
      )}
  ...
  if (ai?.wordFamily?.base?.word) {
    ...
    return {
      base: {
        ...
        ...(wf.base.phonetic?.trim()
          ? { phonetic: wf.base.phonetic.trim() }
          : {}),
      },
...
export default function ReviewCards() {
  ...
  useEffect(() => {
    if (!session?.sessionId || session.cards.length === 0 || session.skipAi)
      return;
  ...
  const aiContent = session.aiContent[card.id] as
    | CardAIContent
    | null
    | undefined;
  ...
  const handleRate = (rating: "easy" | "again") => {
    ...
    rateCard(
      card.id,
      rating,
      cardType === "spelling" ? spellingAnswer : undefined,
    );
    setFlipped(false);
    setSlashState("idle");
    setSavedSyn([]);
  ...
  const performSaveWordFamilyItem = async (item: WordFamilyItem) => {
    ...
    if (
      latest?.wordFamily &&
      flattenWordFamilyItems(latest.wordFamily).some(
        (x) => wordFamilyItemDedupKey(x) === k,
      )
    ) {
      showSaveNotice("已存在");
  ...
  const performSaveWordFamilyAll = async () => {
    ...
    const existingKeys = new Set(
      existing
        ? flattenWordFamilyItems(existing).map(wordFamilyItemDedupKey)
        : [],
    );
    let dup = 0;
    ...
    if (pending.length === 0) {
      showSaveNotice(
        `新增 0 条，跳过重复 ${dup} 条，失败 0 条`,
        dup > 0 ? "warning" : "success",
      );
      return;
    }
    const baseWf = ensureWordFamilyForSave(card, ai, latest);
    ...
    const ok = await updateNote(card.id, { wordFamily: merged });
    if (ok) {
      setSavedWordFamilyKeys((p) => [
        ...p,
        ...pending.map(wordFamilyItemDedupKey),
      ]);
      incrementSavedExtensions();
      showSaveNotice(
        `新增 ${pending.length} 条，跳过重复 ${dup} 条，失败 0 条`,
        "success",
      );
    } else {
      showSaveNotice(
        `新增 0 条，跳过重复 ${dup} 条，失败 ${pending.length} 条`,
      );
    }
  };
  ...
  const performSaveRootItem = async (item: WordFamilyItem) => {
    ...
    const latest = useAppStore.getState().notes.find((n) => n.id === card.id);
    if (
      latest?.wordFamily?.rootDerived?.some(
        (x) => wordFamilyItemDedupKey(x) === k,
      )
    ) {
      showSaveNotice("已存在");
  ...
  const performSaveRootAll = async () => {
    ...
    const existing = latest?.wordFamily;
    const existingKeys = new Set(
      existing?.rootDerived
        ? existing.rootDerived.map(wordFamilyItemDedupKey)
        : [],
    );
    let dup = 0;
    const pending: WordFamilyItem[] = [];
    ...
    if (pending.length === 0) {
      showSaveNotice(
        dup > 0 ? `全部已存在，跳过重复 ${dup} 条` : "暂无可存入的词根派生",
        "warning",
      );
      return;
    }
    ...
    <Layout title="复习">
            ...
            <div>
              ...
              <ul className="text-sm text-text-secondary space-y-2 max-h-40 overflow-y-auto pl-1">
                {assocDedup.conflicts.map((line) => (
                  <li
                    key={line}
                    className="border-l-2 border-amber-500/50 pl-2.5 leading-snug"
                  >
                    {line}
                  </li>
                ))}
        ...
        {showExitConfirm && (
          ...
          >
            <motion.div
              ...
              onClick={(e) => e.stopPropagation()}
              className="w-[min(100vw-2rem,22rem)] max-w-md rounded-2xl border p-6 flex flex-col gap-4"
              style={{
                background: "#1c1c20",
                borderColor: "#3f3f46",
                boxShadow: "0 24px 48px rgba(0,0,0,0.6)",
            ...
            >
              <div>
                ...
                <p className="text-sm text-text-dim leading-relaxed">
                  退出后，当前这一轮复习会话会结束；已经评分（完成翻卡后你的选择）的卡片会照常计入复习与统计。
                </p>
                <p className="text-sm text-text-dim leading-relaxed mt-2">
                  尚未评分的卡片不会记作本轮结果，之后仍可在「复习」里再次开始。
                </p>
              </div>
      ...
      <div className="flex flex-col h-[calc(100vh-3.5rem)] min-h-0 overflow-hidden">
        ...
        <div className="relative min-h-0 flex flex-1 flex-col">
          {/* Card area：全宽；AI 预热为 absolute 悬浮层，不挤占横向空间 */}
          <div className="flex flex-col items-center gap-6 px-6 py-6 sm:px-10 lg:px-12 flex-1 min-h-0 overflow-y-auto">
            {/* Card wrapper */}
                  ...
                  {slashState === "slashing" && (
                    <div className="absolute inset-0 z-30 pointer-events-none">
                      ...
                      <motion.div
                        ...
                        style={{
                          clipPath:
                            "polygon(54% 0, 100% 0, 100% 100%, 32% 100%)",
                        }}
                      />
                      <motion.div
                    ...
                    >
                      ...
                      >
                        <CardBack
                          ...
                          savedAnt={savedAnt}
                          onSaveSyn={(p) => {
                            void handleSaveSyn(p);
                          }}
                          onSaveAnt={(p) => {
                            void handleSaveAnt(p);
                          }}
                          spellingAnswer={spellingAnswer}
                          ...
                          onSavePos={(item) => {
                            void performSavePos(item);
                          }}
                          onSaveConf={(g) => {
                            void performSaveConf(g);
                          }}
                          onSaveWordFamilyItem={(item) => {
                            void performSaveWordFamilyItem(item);
                          }}
                          onSaveWordFamilyAll={() => {
                            void performSaveWordFamilyAll();
                          }}
                          onSaveRootItem={(item) => {
                            void performSaveRootItem(item);
                          }}
                          onSaveRootAll={() => {
                            void performSaveRootAll();
                          }}
                          savedPosKeys={savedPosKeys}
                          savedConfKeys={savedConfKeys}
                          savedWordFamilyKeys={savedWordFamilyKeys}
          ...
          {!session.skipAi && (
            <div className="pointer-events-none absolute inset-0 z-20 flex items-center justify-end pr-2 sm:pr-5">
              <div className="pointer-events-auto max-h-[min(560px,78vh)] shrink-0">
                <ReviewAIWarmupBar variant="floating" />
              </div>
            </div>
          )}
        </div>
