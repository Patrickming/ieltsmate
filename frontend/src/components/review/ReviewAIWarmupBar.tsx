import { AlertCircle, CheckCircle2, Layers, Loader2, RefreshCw, Sparkles } from 'lucide-react'
...
function categoryToCardType(category: Category): 'word-speech' | 'phrase' | 'synonym' | 'sentence' | 'spelling' {
...
type SegmentItem = { id: string; label: string; status: 'loading' | 'pending' | 'ok' | 'failed' }
  ...
  return segments.map((seg, idx) => {
    ...
    const baseH =
      mode === 'vertical'
        ...
        : 'relative flex-1 min-w-0 transition-all duration-300 ease-out ring-1 ring-white/[0.04] ' + layout.cellRounded
    ...
    if (mode === 'horizontal') {
      ...
      if (seg.status === 'loading') {
        return (
          <motion.div key={seg.id} layout title={title} className={baseH + ' overflow-hidden bg-primary/25'}>
            <motion.div
    ...
    // vertical：序号 + 短横条 + 单词/短语标签，填满右侧留白
    const row = (inner: ReactNode) => (
      <div key={seg.id} className="flex items-center gap-2 w-full min-w-0" title={title}>
        <span className="w-4 shrink-0 text-right font-mono text-[9px] tabular-nums text-text-subtle/90">{seq}</span>
        {inner}
        <span className="flex-1 truncate text-[10px] text-text-dim/50">{seg.label}</span>
      </div>
    )
    ...
    if (seg.status === 'loading') {
      return row(
        <motion.div layout className={baseH + ' overflow-hidden bg-primary/25'}>
          <motion.div
            className="absolute inset-0 bg-gradient-to-r from-transparent via-primary/55 to-transparent"
            animate={{ x: ['-100%', '100%'] }}
            transition={{ repeat: Infinity, duration: 1.15, ease: 'linear' }}
    ...
    return row(<div className={baseH + ' bg-zinc-700/55'} />)
  })
}
...
export function ReviewAIWarmupBar({ variant = 'floating' }: ReviewAIWarmupBarProps) {
  ...
  const snapshot = useMemo(() => {
    if (!reviewSession?.sessionId || reviewSession.skipAi || reviewSession.cards.length === 0) return null
    ...
    for (const note of windowCards) {
      const st = cellStatus(note.id, aiContent as Record<string, unknown>, aiLoading)
      if (st === 'ok') ok += 1
      else if (st === 'failed') {
        failed += 1
    ...
    const segments = windowCards.map((note) => ({
      ...
      status: cellStatus(note.id, aiContent as Record<string, unknown>, aiLoading),
    }))
  ...
  const { total, doneFull, failed, loading, pending, failedNotes, segments, start, end } = snapshot
  ...
  const ringChart = (sizeClass: string) => (
  ...
  const renderStatPills = (wrapClass: string) => (
    <div className={wrapClass}>
      <span
        className={`inline-flex items-center rounded-full border border-emerald-500/25 bg-emerald-500/[0.12] font-medium text-emerald-200/95 ${layout.statPill}`}
      >
        完整 {doneFull}/{total}
      </span>
      {loading > 0 && (
        <span
          className={`inline-flex items-center gap-1 rounded-full border border-primary/30 bg-primary/[0.12] font-medium text-primary ${layout.statPill}`}
        >
          <Loader2 size={11} className="animate-spin shrink-0 opacity-90" />
          {loading}
        </span>
      )}
      {pending > 0 && (
        <span
          className={`inline-flex items-center rounded-full border border-zinc-600/50 bg-zinc-800/60 font-medium text-text-dim ${layout.statPill}`}
        >
          待拉取 {pending}
        </span>
      )}
      {failed > 0 && (
        <span
          className={`inline-flex items-center gap-0.5 rounded-full border border-amber-500/35 bg-amber-500/[0.12] font-medium text-amber-100/95 ${layout.statPill}`}
        >
          <AlertCircle size={11} className="shrink-0 opacity-90" />
          降级 {failed}
        </span>
      )}
    </div>
  ...
  const failedSection = failedNotes.length > 0 && (
    <div className={`flex flex-col gap-2 ${variant === 'floating' ? 'mt-2' : 'mt-3'}`}>
  ...
  const readyLine = failed === 0 && loading === 0 && pending === 0 && total > 0 && (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className={`flex items-center gap-1.5 text-[10px] font-medium text-emerald-400/75 ${variant === 'floating' ? 'mt-2 shrink-0 border-t border-white/[0.05] pt-2' : 'mt-2.5'}`}
    >
  ...
  if (variant === 'floating') {
    return (
      ...
      >
        {bgDecor}
        <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden px-3 py-3">
          <div className="mb-2 flex shrink-0 items-start gap-2">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1">
                ...
                <span className="text-[11px] font-semibold leading-tight text-text-secondary">滑动窗口 · AI 预热</span>
              </div>
              <p className={`mt-0.5 text-text-dim ${layout.labelClass}`}>
                第 {start + 1}–{end} 张 · 共{' '}
                <span className="tabular-nums font-medium text-text-muted">{total}</span> 条
                {total !== reviewPrepareBatchSize && total < reviewPrepareBatchSize && (
                  <span className="text-text-subtle">（将尽）</span>
                )}
              </p>
            </div>
            {ringChart('h-10 w-10')}
          ...
          <div className="mb-2 shrink-0">{renderStatPills('flex flex-wrap gap-1.5')}</div>

          <div className="flex min-h-0 flex-1 flex-col rounded-xl border border-white/[0.07] bg-black/30 p-2 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] backdrop-blur-sm">
            <div className="flex min-h-0 flex-1 flex-col gap-1 overflow-y-auto overscroll-contain pr-0.5 [scrollbar-gutter:stable]">
              <WarmupSegmentCells
                segments={segments}
                start={start}
                layout={layout}
        ...
        <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className={`mt-0.5 text-text-dim ${layout.labelClass} truncate`}>
                第 {start + 1}–{end} 张 · 本窗口共{' '}
                <span className="tabular-nums text-text-muted font-medium">{total}</span> 条
                {total !== reviewPrepareBatchSize && total < reviewPrepareBatchSize && (
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            {renderStatPills('flex flex-wrap gap-1.5 justify-end max-w-[min(100%,280px)] sm:max-w-none')}
          </div>
        </div>
